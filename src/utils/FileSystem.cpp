...
bool FileSystem::pathCopy(const std::string& sourcePath, const std::string& destinationPath) {
	CONSOLE_LOG_INFO("Copying path from: ", sourcePath, " to: ", destinationPath);
	std::error_code ec;
	std::filesystem::copy(sourcePath, destinationPath, std::filesystem::copy_options::overwrite_existing, ec);
	if (ec) {
		CONSOLE_LOG_ERROR("Copy failed: ", ec.message());
		return false;
	}
	return true;
}
bool FileSystem::pathRemoveAll(const std::string& path) {
	CONSOLE_LOG_INFO("Removing path: ", path);
	if (std::filesystem::remove_all(path)) {
		return true;
	}
	CONSOLE_LOG_ERROR("Failed to remove path: ", path);
	return false;
}

bool FileSystem::fileExist(const std::string& filePath) {
	CONSOLE_LOG_INFO("Checking if file exists: ", filePath);
	return std::filesystem::exists(filePath);
}
bool FileSystem::fileRemove(const std::string& filePath) {
	CONSOLE_LOG_INFO("Removing file: ", filePath);
	if (std::filesystem::remove(filePath)) {
		return true;
	}
	CONSOLE_LOG_ERROR("Failed to remove file: ", filePath);
	return false;
}
bool FileSystem::fileCopy(const std::string& sourcePath, const std::string& destinationPath) {
	CONSOLE_LOG_INFO("Copying file at path: ", sourcePath, " to path: ", destinationPath);
	if (std::filesystem::copy_file(sourcePath, destinationPath)) {
		return true;
	}
	CONSOLE_LOG_ERROR("Failed to copy file at path: ", sourcePath, " to path: ", destinationPath);
	return false;
}
std::string FileSystem::fileGetName(const std::string& filePath) {
	CONSOLE_LOG_INFO("Getting file name for: ", filePath);
...
std::string FileSystem::getFileBuffer(const std::string& filePath) {
	...
	if (!file.is_open()) {
		CONSOLE_LOG_ERROR("Failed to open file: ", filePath);
		return "";
	}
...
std::string FileSystem::getExecutablePath() {
	...
	if (GetModuleFileNameA(nullptr, buffer, MAX_PATH) == 0) {
		CONSOLE_LOG_ERROR("Failed to retrieve executable path.");
	}
	return std::string(buffer);
}
std::string FileSystem::getExecutableDirectory() {
	CONSOLE_LOG_INFO("Fetching the executable directory.");
...
std::string FileSystem::getUserFolder() {
	...
	if (FAILED(SHGetKnownFolderPath(FOLDERID_RoamingAppData, 0, nullptr, &path))) {
		CONSOLE_LOG_ERROR("Failed to retrieve the user folder path.");
	}
	std::wstring ws(path);
	CoTaskMemFree(path);
...
std::string FileSystem::getTempFolder() {
	...
	if (result == 0 || result > MAX_PATH) {
		CONSOLE_LOG_ERROR("Failed to retrieve the temporary folder path.");
	}
	return std::string(buffer);
}
...
void FileSystem::openFileExplorer(const std::string& path) {
	...
	if ((intptr_t)ShellExecuteA(nullptr, "open", "explorer.exe", command.c_str(), nullptr, SW_SHOWNORMAL) <= 32) {
		CONSOLE_LOG_ERROR("Failed to open file explorer for path: ", path);
	}
}
